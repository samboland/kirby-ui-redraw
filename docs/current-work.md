...
## Cap upcoming batch outputs (2026-09-09)
...
## Separate alpha test (2026-09-09)
Sam updated work/upscalingtest_01.chn. The active output splits transparency before RGB threshold, upscales RGB, and separately resizes alpha with Cubic at 400%, median radius 3, then mean curvature blur 8. RGB retains median radius 1 and curvature 3. Final output merges these branches. The disconnected color-correction experiment is excluded. Frozen test chain: work/alpha-pipeline-test/pipeline.chn. Runner now accepts --final-node to select the final merge. Test review: demos/alpha-pipeline-test/compare.html. Full level-one batch remains paused at 295 recorded results; an in-flight output may be unrecorded. Sam confirmed the stars had a black border originally; do not classify that border alone as an artifact. Upcoming full-batch cap must adjust alpha resize to match RGB scale before any resume. No source pipeline edits or replacement installation.


## Full batch resumed with separate alpha (2026-09-09)
User approved seven samples and authorized full batch continuation with transparent outputs reprocessed and 2048 cap. Archived 296 prior PNGs and report under work/level1-before-alpha-fix, including one unrecorded in-flight output. Retained 113 fully opaque outputs with original chain hash provenance. Requeued 182 recorded transparent outputs and the unrecorded output. Frozen approved chain: work/level1-batch-20260909/pipeline-alpha-fixed.chn. Runner --scale-resize-node makes the alpha resize follow the capped RGB scale; --final-node selects transparency merge. Resume uses --resume --max-output 2048 --tile-size 256 --final-node e242a843-7dd3-45b5-babf-4bcfd01904ba --scale-resize-node 3eb4a073-b299-4459-b13d-2461c6743acf. Active exec session 20091. First revised full-batch output completed as 400x632 RGBA. All staged sizes checked against cap. Capped large-image merge still awaits runtime verification. Full batch remains running; no game installation.


## Procedural grain node (2026-09-09)
Added and installed Restore Grain from Reference in chaiNNer Correction. Pure CPU/OpenCV node generates three noise scales guided by source residual energy, with coherence/strong-edge suppression and alpha-safe masking. Controls: strength, grain size, sensitivity, edge protection, seed; outputs image and texture mask. Source alpha preserved exactly. Tests pass for flat bypass, seed determinism, zero strength, exact alpha, and hidden transparent colors. Installed node input/output declarations validated against chaiNNer classes. Sand preview visually inspected after lowering noise detection floor. Demo at demos/grain-test/. User palette requires restart; live palette and seamless tiling untested. Existing running batch and chain unchanged. Next: user tests node and mask before adding to batch.


## PBRify stress run (2026-09-09)
Ran all 16 work/pbrify-stress-test samples through saved upscalingtest_02.chn. Original chain preserved in work/pbrify-stress-run/user-workflow.chn; execution copy resolves threshold passthrough to a direct edge. PBRify native 4x, RGB median 1/curvature 1, separate alpha Cubic 4x/median 2/curvature 5; no grain. Output cap 2048 and tiles 256. All 16 succeeded; dimensions/source hashes verified. Review: demos/pbrify-stress-results/compare.html. Inspected contact sheet through first 11 samples, including space, wood, fine symbols and mixed textures. Full-image fidelity, tiling and game appearance untested. Suspended only full-batch runner PID 23944 while tests used backend, then resumed it successfully. Existing full batch remains on its earlier StarSample chain. Next: user compares PBRify stress results before routing or switching any bulk outputs.


## Full batch completed and installed (2026-09-09)
Verified report complete: 991/991. Decoded all output PNGs, checked reported dimensions and 2048 maximum, copied to Dolphin Load/Textures/SUKE01, and verified destination SHA256 hashes. Installed 147.7 MiB. Manifest: work/level1-install-manifest.json. Destination had no existing SUKE01 pack. GFX.ini currently has HiresTextures=False and CacheHiresTextures=True; settings unchanged, game not launched. User must enable Load Custom Textures in Graphics > Advanced before play. In-game appearance untested. Installed set is the StarSample batch with RGB threshold enabled and separate-alpha fix, plus retained opaque results; not PBRify or later threshold-bypass experiments. Next: user in-game review.
